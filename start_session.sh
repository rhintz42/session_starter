...
if echo "$HOST" | grep -q "$SUBSTRING"; then
    ...
    avconv -f x11grab -r 4 -s $RESOLUTION_SIZE -i :0.0 -vcodec libx264 -preset ultrafast -threads 4 -y $1.mp4  # The $1 is the first argument passed to this script
    ...
    avconv -f x11grab -r 4 -s $RESOLUTION_SIZE -i :0.0 -vcodec libx264 -preset ultrafast -threads 4 -y $1.mp4
else
